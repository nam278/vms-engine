...
/**
 * @brief Builds nvmsgconv element for event message conversion.
 *
 * Index encoding: output_index * 100 + element_index.
 * Reads config.outputs[output_index].elements[element_index].
 */
class MsgconvBuilder : public engine::core::builders::IElementBuilder {
   public:
    explicit MsgconvBuilder(GstElement* bin);
