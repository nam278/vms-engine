...
extern "C" bool NvDsInferParseYoloFace(std::vector<NvDsInferLayerInfo> const& outputLayersInfo,
...
static float computeIoU(NvDsInferInstanceMaskInfo& bbox1, NvDsInferInstanceMaskInfo& bbox2) {
    float overlapX =
...
static std::vector<NvDsInferInstanceMaskInfo> nonMaximumSuppression(
...
static std::vector<NvDsInferInstanceMaskInfo> nmsAllClasses(
    std::vector<NvDsInferInstanceMaskInfo>& binfo) {
    std::vector<NvDsInferInstanceMaskInfo> result = nonMaximumSuppression(binfo);
    return result;
}

static void addFaceProposal(const float* output, size_t channelsSize, uint netW, uint netH,
                            size_t n, NvDsInferInstanceMaskInfo& b) {
...
static void addBBoxProposal(float x1, float y1, float x2, float y2, uint netW, uint netH,
                            int maxIndex, float maxProb, NvDsInferInstanceMaskInfo& b) {
    x1 = clamp(x1, 0, netW);
    y1 = clamp(y1, 0, netH);
    x2 = clamp(x2, 0, netW);
    y2 = clamp(y2, 0, netH);
...
static std::vector<NvDsInferInstanceMaskInfo> decodeTensorYoloFace(
    const float* output, size_t outputSize, size_t channelsSize, uint netW, uint netH,
...
static bool NvDsInferParseCustomYoloFace(std::vector<NvDsInferLayerInfo> const& outputLayersInfo,
    ...
    std::vector<NvDsInferInstanceMaskInfo> objects = decodeTensorYoloFace(
        (const float*)(output.buffer), outputSize, channelsSize, networkInfo.width,
